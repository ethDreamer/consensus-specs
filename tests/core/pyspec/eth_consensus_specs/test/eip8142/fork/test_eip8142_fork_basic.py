...
from eth_consensus_specs.test.helpers.eip8142.fork import (
...
@with_phases(phases=[HEZE], other_phases=[EIP8142])
@spec_test
@with_state
@with_meta_tags(EIP8142_FORK_TEST_META_TAGS)
def test_fork_base_state(spec, phases, state):
    yield from run_fork_test(phases[EIP8142], state)
